...
better tools to handle growing volumes more efficiently.

This proposal recommends deploying a locally-hosted AI-powered
grant management assistant that supports intake screening,
eligibility scoring, applicant ranking, contract generation,
progress monitoring, and closure evaluation — while keeping all
data on-premise within JEDCO's infrastructure.

The system handles routine administrative steps so program
...
- With thousands of applications, AI can provide a consistent
  baseline score that officers review and refine with their
  professional judgment
- Shortlisting and ranking involve intensive staff coordination
  across many applications — AI can prepare ranked lists for review
- Progress reports involve individual review of each project — AI can
  generate draft summaries for officer approval
- Closure evaluations could benefit from a standardized
  scoring template — AI can calculate and pre-fill

1.2 Scale of the Opportunity
- Tattweer and Start Business together can generate 1,000-4,000+
  applications in a single cycle
- Each full review cycle involves thorough evaluation across administrative,
  financial, technical, and field stages — AI assistance in the early stages
  allows this expert capacity to be focused on the most impactful work
...
grant lifecycle — from intake through closure — using locally
deployed language models and workflow automation software.

All processing occurs on JEDCO-owned servers.
...
#### STAGE 3: MULTI-CRITERIA SCORING
  - Qualifying applications are scored 0-100 against weighted criteria:
    ...
    * Innovation / productivity impact (20%)
    * Financial capacity and co-investment (20%)
    * Documentation quality (15%)
  - Scores are computed consistently using the AI model
...
#### STAGE 4: RANKING & SHORTLISTING
  ...
  - Tie-breaking rules applied automatically (per program criteria)
  - Borderline band (configurable ± 5 points from cutoff)
...
#### STAGE 5: INVITATION & CONTRACT GENERATION
  ...
  - Contract generated from pre-approved template, populated with:
    * Applicant details
    * Approved grant amount and installment schedule
    * Action Plan milestones from application
    * Reporting deadlines
...
#### STAGE 6: PROGRESS MONITORING
  ...
  - AI reads report and produces summary per project:
    * Milestone status (on track / delayed / at risk)
    * Financial burn rate vs approved budget
    * Risk flags
...
#### STAGE 7: FINAL EVALUATION & CLOSURE
  - At contract end, AI compares actual vs target KPIs:
    * Employment numbers
    * Production capacity
    * Certification / market access targets
    * Financial expenditure
  - System calculates project completion % and financial completion %
...
--------------------|-------------------|---------------------------
Language Model      | Ollama (local)    | AI inference engine
AI Model            | Qwen2.5:7B        | Arabic + English NLP
                    | (or Llama3.1:8B)  |
Document Q&A        | AnythingLLM       | Document reading interface
Workflow Automation | n8n               | Pipeline orchestration
Database            | PostgreSQL        | Application data storage
Web Interface       | React / Next.js   | Officer dashboard
...
MINIMUM (for up to 1,000 applications/cycle):
  ...
  - GPU: NVIDIA RTX 3090 24GB VRAM (for local model inference)
  - OS: Ubuntu 22.04 LTS

RECOMMENDED (for 4,000+ applications/cycle):
  - Server: 64GB RAM, 16-core CPU, 2TB NVMe SSD
  - GPU: NVIDIA A10 24GB or 2x RTX 4090
  - Redundant storage + daily backups
  - Dedicated network VLAN for application data

CLOUD ALTERNATIVE (if on-premise GPU not available):
  - Deploy on Microsoft Azure Government (Jordan region)
  - Models: Azure OpenAI Service (Arabic-capable GPT-4o)
...
- Role-based access control: intake, evaluation, approvals
...
#### PHASE 1 — PILOT (3 months)
  ...
  - Train 2-3 JEDCO champion officers on the system
...
------------------------------------|-------------------------------------------
Document verification               | AI flags missing items; officer confirms
Eligibility screening               | AI pre-checks; officer reviews and decides
Scoring 4,000 applications          | AI provides baseline; officer refines
Progress report summaries           | AI drafts one-page summary; officer reviews
...
ITEM                            | COST (JOD)
--------------------------------|------------------
Hardware (server + GPU)         | 8,000 - 15,000
Software setup & configuration  | 5,000 - 10,000
Custom dashboard development    | 15,000 - 25,000
Integration with existing portal| 5,000 - 8,000
Staff training (2 days)         | 2,000 - 3,000
Year 1 support & maintenance    | 5,000 - 8,000
--------------------------------|------------------
TOTAL ESTIMATE                  | 40,000 - 69,000 JOD
...
Sector priority         | 20%    | Low-value | Mixed   | Top 5 sectors
                        |        | sector    | sector  | (ICT, food,
                        |        |           |         | pharma, etc.)
Employment creation     | 25%    | 0-1 jobs  | 3-5     | 6+ new jobs
Innovation component    | 20%    | None      | Partial | Core to project
...
- Measure: agreement rate between AI preliminary assessment and evaluator
  recommendation, processing time per application, and consistency of
